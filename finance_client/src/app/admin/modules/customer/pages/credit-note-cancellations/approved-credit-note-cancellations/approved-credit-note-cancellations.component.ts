...
@Component({
  selector: "app-approved-credit-note-cancellations",
  templateUrl: "./approved-credit-note-cancellations.component.html",
  styleUrls: ["./approved-credit-note-cancellations.component.sass"],
})
export class ApprovedCreditNoteCancellationsComponent
  ...
  implements OnInit
{
  ...
  getInvoices() {
    ...
    this.creditNoteCancellationService
      .fetchByCancellationStatus(params)
  ...
  postCreditNoteCancellation(creditNote) {
    console.log("", creditNote);

    let cancellation;

    cancellation = {
      invoiceApplyCategoryCode: creditNote.invoiceApplyCategoryCode,
    ...
    this.creditNoteCancellationService.cancelCreditNote(cancellation).pipe(takeUntil(this.subject)).subscribe(res => {
      console.log(res);

      this.getInvoices();
    }, err => {

      this.snackbar.showNotification("snackbar-danger", "Unable to cancel credit note, please try again later !")
      this.getInvoices();
