...
@Component({
  ...
  styleUrls: ["./paid-invoices.component.sass"],
})
export class PaidInvoicesComponent extends BaseComponent implements OnInit {
  ...
  navigateToInvoiceDetails(invoiceno) {
    this.router.navigate([
      ...
      {
        state: {
          action: "Navigation From Paid Invoices",
        },
      },);
  }
  ...
  postInvoiceToUra(localInvoiceNo, buyerTin) {
    this.loading = true;
    this.invoiceService
      .uploadInvoice(localInvoiceNo, buyerTin)
      .pipe(takeUntil(this.subject))
      .subscribe(
        (result) => {
