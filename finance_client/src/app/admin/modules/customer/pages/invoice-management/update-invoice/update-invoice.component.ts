...
import { DiscountInvoiceItemsComponent } from "../add-invoice/dialog/discount-invoice-items/discount-invoice-items.component";
...
@Component({
  ...
  styleUrls: ["./update-invoice.component.sass"],
})
export class UpdateInvoiceComponent extends BaseComponent implements OnInit {
  isLinear = false;
  ...
  constructor(
    ...
    if (navigationExtras && navigationExtras.state) {
      ...
      if (state.action === "Navigation Back From Preview") {
        console.log("Router State", state);
        console.log("Parsed", JSON.parse(state.invoice));
        this.invoice = state.invoice ? JSON.parse(state.invoice) : {};
      } else if (state.action === "Assign customer PO") {
        console.log("Router State", state);
        console.log("Parsed", JSON.parse(state.customerDetails));
        this.customer = state.customerDetails
          ? JSON.parse(state.customerDetails)
          : {};
      } else if (state.action === "Update Invoice") {
        console.log("Router State", state);
        console.log("Parsed", JSON.parse(state.invoice));
        this.invoiceDetails = state.invoice ? JSON.parse(state.invoice) : {};
      } else {
        this.invoice = {};
        this.customer = {};
      }
  ...
  ngOnInit() {
    ...
    this.invoiceno = this.invoiceDetails.localInvoiceNo;
    console.log("localInvoiceNo: ", this.invoiceno);
    forkJoin([
      this.invoiceService.fetchInvoiceBasicInformationByIdBeforePost(
        this.invoiceno
      ),
      this.invoiceService.fetchSellerDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchBuyerDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchImportSellerDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchGoodsDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchPayWayInfoByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchSummaryDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
      this.invoiceService.fetchTaxDetailsByInvoiceIdBeforePosting(
        this.invoiceno
      ),
    ]).subscribe(
      ([
        ...
        this.taxDetailsInfo = taxDetails;

        this.theLocalInvoiceNo = this.basicInformation.localInvoiceNo

        console.log("this.localInvoiceNo = ", this.basicInformation.localInvoiceNo);
        ...
        // this.pushGoodsDetails();
        this.fetchSupplimentaryDetails();

        this.invoiceItemArray = this.goodsDetails;
        this.taxDetailsArray = this.taxDetailsInfo;
        this.invoiceTaxSummarryArray = this.taxDetailsInfo;

        //  this.invoiceItemArray.forEach((item) => {
        //     this.getInvoiceItems(item);
        //   });
  ...
  //         "discountTotal": "",
  //         "exciseFlag": "2",
  //         "goodsCategoryId": "84131519",
  //         "item": "Bancassurance services",
  //         "itemCode": "44855",
  //         "orderNumber": 0,
  //         "qty": 1,
  //         "tax": "2974.58",
  ...
  createGenarateInvoiceForm() {
    this.generateInvoiceForm = this.fb.group({
      ...
      basicInformation: this.fb.group({
        id: [this.basicInformation.id],
        currency: [this.basicInformation.currency],
        dataSource: [this.basicInformation.dataSource],
        deviceNo: [this.basicInformation.deviceNo],
        invoiceIndustryCode: [this.basicInformation.invoiceIndustryCode],
        invoiceKind: [this.basicInformation.invoiceKind],
        invoiceType: [this.basicInformation.invoiceType],
        issuedDate: [this.basicInformation.issuedDate],
        operator: [this.username],
        oriInvoiceId: [this.basicInformation.oriInvoiceId],
        dateOfIssue: [new Date()],
        localInvoiceNo: this.theLocalInvoiceNo
      }),
      buyerDetails: this.fb.group({
        id: [this.buyerDetails.id],
        buyerCitizenship: [
          this.buyerDetails.buyerCitizenship,
          [Validators.required],
        ],
        buyerLegalName: [
          this.buyerDetails.buyerLegalName,
          [Validators.required],
        ],
        buyerMobilePhone: [
          this.buyerDetails.buyerMobilePhone,
          [Validators.required],
        ],
        buyerPassportNum: [
          this.buyerDetails.buyerPassportNum,
          [Validators.required],
        ],
        buyerSector: [this.buyerDetails.buyerSector, [Validators.required]],
        buyerTin: [this.buyerDetails.buyerTin, [Validators.required]],
        buyerType: [this.buyerDetails.buyerType, [Validators.required]],
        localInvoiceNo: this.theLocalInvoiceNo
      }),
      importServicesSeller: this.fb.group({
        id: [this.importSellerDetails.id],
        importBusinessName: [this.importSellerDetails.importBusinessName],
        importEmailAddress: [this.importSellerDetails.importEmailAddress],
        importContactNumber: [this.importSellerDetails.importContactNumber],
        importAddress: [this.importSellerDetails.importAddress],
        importInvoiceDate: [new Date()],
        importAttachmentName: [this.importSellerDetails.importAttachmentName],
        importAttachmentContent: [
          this.importSellerDetails.importAttachmentContent,
        ],
        localInvoiceNo: this.theLocalInvoiceNo
      }),
      extend: {
      ...
      //  URA RESPONCE: RES CODE: 1344 RES DESC| summary-->taxAmount:Must be equal to the sum of all taxAmount's in taxDetails!


      sellerDetails: this.fb.group({
        id: [this.sellerDetails.id],
        address: [this.sellerDetails.address, [Validators.required]],
        businessName: [this.sellerDetails.businessName, [Validators.required]],
        emailAddress: [this.sellerDetails.emailAddress, [Validators.required]],
        legalName: [this.sellerDetails.legalName, [Validators.required]],
        linePhone: [this.sellerDetails.linePhone, [Validators.required]],
        mobilePhone: [this.sellerDetails.mobilePhone, [Validators.required]],
        referenceNo: [this.sellerDetails.referenceNo, [Validators.required]],
        tin: [this.sellerDetails.tin, [Validators.required]],
        localInvoiceNo: this.theLocalInvoiceNo
      }),
      summary: this.fb.group({
        id: [this.summaryDetails.id],
        grossAmount: [this.summaryDetails.grossAmount, [Validators.required]],
        invoiceID: [""],
        itemCount: [this.summaryDetails.itemCount, [Validators.required]],
        modeCode: [this.summaryDetails.modeCode, [Validators.required]],
        netAmount: [this.summaryDetails.netAmount, [Validators.required]],
        qrCode: ["", [Validators.required]],
        remarks: [this.summaryDetails.remarks, [Validators.required]],
        taxAmount: [this.summaryDetails.taxAmount, [Validators.required]],
        localInvoiceNo: [this.theLocalInvoiceNo]
      }),
      airlineGoods: new FormArray([]),
      goodsDetails: new FormArray([]),
      payWay: [this.payWayInfo],
      customerid: this.buyerDetails.buyerTin,
    ...
    if (this.isBuyerOrSellerANonResident) {
      ...
      this.generateInvoiceForm.patchValue({
        ...
        taxDetails: this.taxDetailsInfo
      });
    }
  ...
  fetchSupplimentaryDetails() {
    ...
    this.getInvoiceItems(this.goodsDetails);
  }

  // pushGoodsDetails() {
  //   this.goodsDetails.forEach((good) => {
  //     this.goodsDetailsForm.patchValue({
  //       id: good.id,
  //       deemedFlag: good.deemedFlag,
  //       discountFlag: good.discountFlag,
  //       discountTaxRate: good.discountTaxRate,
  //       discountTotal: good.discountTotal,
  //       exciseFlag: good.exciseFlag,
  ...
  //       taxId: good.taxId,
  //       taxCode: good.taxCode,
  //     });

  //     this.addInvoiceItem();

  //     this.invoiceItemArray.push(good);
  //   });
  // }
  // pushPayWayInfo() {
  //   this.modeOfPaymentForm.patchValue({
  //     id: this.payWayInfo[0].id,
  //     orderNumber: this.payWayInfo[0].orderNumber,
  //     paymentAmount: this.payWayInfo[0].paymentAmount,
  //     paymentMode: this.payWayInfo[0].paymentMode,
  //   });
  // }
  ...
  generateInvoice() {
    ...
    if (!this.isBuyerOrSellerANonResident) {
      ...
      this.generateInvoiceForm.patchValue({
        customerid: this.sellerDetails.tin,
      });
    }
    console.log(
      "this.invoiceForm: ",
    ...
    this.invoiceService
      .updateInvoiceDetails(this.generateInvoiceForm.value)
      .pipe(takeUntil(this.subject))
      .subscribe(
        (result) => {
