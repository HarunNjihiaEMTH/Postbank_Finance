...
export class AllItemsComponent extends BaseComponent implements OnInit {
  items: Item[] = [];
  ...
  //     ["/user/supplier/expenses-management/update-expense"],
  //     {
  //       state: {
  ...
  editStockCall(data) {
   // this.hasAccess = this.accessControlService.hasPrivilege(["Update Roles"]);
    ...
    localStorage.setItem("editStockData", JSON.stringify(data));
      this.router.navigate([`/admin/customer/items-management/add-item`], {
        state: {
