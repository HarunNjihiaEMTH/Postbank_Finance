...
@Component({
  selector: 'app-approved-credit-notes',
  templateUrl: './approved-credit-notes.component.html',
  styleUrls: ['./approved-credit-notes.component.sass']
})
export class ApprovedCreditNotesComponent extends BaseComponent implements OnInit {
      ...
      .subscribe(
        (result) => {
          ...
          if (this.creditNotes.length > 0) {
            this.approvedCreditNotes = [];

            this.creditNotes.forEach(creditNote => {
              
              if(creditNote.status == "Approved" && creditNote.uraStatus != "Successful" && creditNote.uraStatus != "Failed"){
                this.approvedCreditNotes.push(creditNote)
              }
            })
            
          }

          if(this.approvedCreditNotes.length > 0){
         

            this.dataSource = new MatTableDataSource<Invoice>(this.approvedCreditNotes.reverse());
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;

            console.log("Approved ", this.approvedCreditNotes)
  ...
  postToURA(row){
    ...
    this.creditNoteService.postToURA(row.oriInvoiceNo).pipe(takeUntil(this.subject)).subscribe(res => {
