...
@Component({
  ...
  styleUrls: ["./expenses-lookup.component.sass"],
})
export class ExpensesLookupComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.data.action == 'single selection expenses'){
      this.selection = new SelectionModel<any>(false, []);
    }
    this.expensesDetails = this.data.data;
    this.expensesArray = this.data.selected;

    console.log("this.expenses: ", this.expensesDetails);

    this.dataSource = new MatTableDataSource<any>(this.expensesDetails);
    this.dataSource.sort = this.sort;

    if (this.expensesArray.length !== 0) {
      this.filter();
      console.log("filter by: ", this.expensesArray);
    }
  }
  ngAfterViewInit() {
  ...
  filter() {
    ...
    this.dataSource.data.forEach((element) => {
      this.expensesArray.forEach((item) => {
        if (item === element.id) {
