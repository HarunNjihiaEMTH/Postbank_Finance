...
export class AccountService {
  ...
  public listActiveAccounts(): Observable<Account[]> {
    const activeAccountsUrl = `${environment.authUrl}/soa/users/view`;

    return this.http.get<Account[]>(activeAccountsUrl);
  }

  public fetchPendingAccounts(): Observable<any[]> {
    const fetchPendingAccountsUrl = `${environment.authUrl}/soa/users/view/pending`;

    return this.http.get<any[]>(fetchPendingAccountsUrl);
  }

  public fetchApprovedAccounts(): Observable<any[]> {
    const fetchApprovedAccountsUrl = `${environment.authUrl}/soa/users/view/approved`;

    return this.http.get<any[]>(fetchApprovedAccountsUrl);
  }

  public fetchRejectedAccounts(): Observable<any[]> {
    const fetchRejectedAccountsUrl = `${environment.authUrl}/soa/users/view/rejected`;
  ...
  public generateAccountReportsPerStatus(params): Observable<any> {
    ...
    const generateAccountReportsPerStatusUrl = `${environment.baseUrl}/api/v1/reports/users/user/status`;
  ...
  public activateUserAccount(userDetails): Observable<{ message: string }> {
    const activateUserAccountUrl = `${environment.authUrl}/soa/users/activateaccount`;
  ...
  deactivateUserAccount(userDetails): Observable<{ message: string }> {
    const deactivateUserAccountUrl = `${environment.authUrl}/soa/users/activateaccount`;
  ...
  public lockUserAccount(userDetails): Observable<{ message: string }> {
    const lockUserUrl = `${environment.authUrl}/soa/users/lockaccount`;
  ...
  public deleteUser(username): Observable<{ message: string }> {
    const deleteUrl = `${environment.authUrl}/soa/users/deleteaccount`;

    return this.http.put<{ message: string }>(deleteUrl, username);
  }

  public restoreDeletedAccount(username): Observable<{ message: string }> {
    const restoreDeletedaccountUrl = `${environment.authUrl}/soa/users/restoreaccount`;
  ...
  public updateUserPassword(passwordDetails): Observable<{ message: string }> {
    const updateUserPasswordUrl = `${environment.authUrl}/soa/users/updatepassword`;
  ...
  public updateFirstTimePassword(
    ...
    const updateUserPasswordUrl = `${environment.authUrl}/ebm/auth/resetpassword`;
  ...
  addLimit(limitConfig: any): Observable<any> {
    return this.http.post(
      `${environment.authUrl}/emtbulksmsapi/ebs/smslimits/add`,
      limitConfig
    );
  }

  public getLimitByUsername(userName): Observable<any> {
    const usersUrl = `${environment.authUrl}/ebs/smslimits/list/${userName}`;

    return this.http.get<any>(usersUrl);
  }

  addEmailLimit(limitConfig: any): Observable<any> {
    return this.http.post(`${environment.authUrl}/soa/limits/add`, limitConfig);
  }

  public updateUserDepartment(user): Observable<{ message: string }> {
    const updateUserUrl = `${environment.authUrl}/soa/users/updatedepartment`;

    return this.http.put<{ message: string }>(updateUserUrl, user);
  }

  getDepartments(): Observable<any[]> {
    return this.http.get<any[]>(`${environment.authUrl}/soa/departments/view`);
  }
  ...
  resendEmail(userId: any) {
    ...
    const resendUrl = `${environment.authUrl}/soa/users/resend-email`;
