...
export class ReportsInvoiceService {
  ...
  generateInvoiceReportPerCustomerPdfReport(params: any): Observable<any> {
    ...
    let API_URL = `${environment.baseUrl}/api/v1/reports/invoice/invoices/customer/`;

    return this.http.get(API_URL, requestOptions).pipe(
      map((response) => {
        console.log("Hey this is Invoice customer report", response);
        return {
          filename: "Report",
