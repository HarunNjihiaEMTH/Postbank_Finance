...
export class SupplierService {
  ...
  addBulkSuppliers(suppliersArray): Observable<{message: string}>{
    const addSuppliersUrl = `${environment.baseUrl}/api/v1/supplier/add/supplier/array`;

    return this.http.post<{message: string}>(addSuppliersUrl, suppliersArray)
  }
