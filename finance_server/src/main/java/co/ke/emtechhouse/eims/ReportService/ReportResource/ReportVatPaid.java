...
import co.ke.emtechhouse.eims.ReportService.ExcelExports.ExcelHeaders.VATCollected;
...
//        String transactiontype = "%";
        ...
        String producttype = "%";
        String status = "%";
        List<SupplierPayment>  paymentsList = reportQueriesRepo.vhtQueryWithParams(fromDate,toDate,transactiontype,finacle_status,supplier_id,producttype, status);
        if(paymentsList.size()>0){
            ...
            ByteArrayInputStream bis = excelExporter.exportVAT(httpresponse,paymentsList, VATCollected.class, fromDate, toDate);
            HttpHeaders headers = new HttpHeaders();
